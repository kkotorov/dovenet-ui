  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const father = parentsFromDB.find(
            (p) => p.ringNumber === fetchedPigeon.fatherRingNumber
        );
        parentsList.push(
            father || {
            id: undefined,
        ...
        const mother = parentsFromDB.find(
            (p) => p.ringNumber === fetchedPigeon.motherRingNumber
        );
        parentsList.push(
            mother || {
            id: undefined,
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="col-span-2 bg-white shadow-md rounded-2xl p-6 border border-gray-100">
            ...
            <div className="space-y-1">
                <div className="text-xs text-gray-500">Loft</div>
                <div
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 rounded-lg overflow-hidden">
                ...
                <tbody className="bg-white divide-y divide-gray-100">
                  {sortedCompetitions.map((entry) => (
                    <tr
                      key={entry.id}
                      className="hover:bg-blue-50 transition cursor-pointer"
                      onClick={() => {
                        if (entry.competition?.id)
                          navigate(`/competitions/${entry.competition.id}`);
                      }}
                    >
                      <td className="px-4 py-3 text-sm text-gray-800">{entry.competition?.name || "-"}</td>
                      <td className="px-4 py-3 text-sm text-gray-800">{entry.place ?? "-"}</td>
                      <td className="px-4 py-3 text-sm text-gray-800">{entry.competition?.date || "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
